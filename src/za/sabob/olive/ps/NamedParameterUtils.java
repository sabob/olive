...
public abstract class NamedParameterUtils {
                ...
                if (j < statement.length && c == ':' && statement[j] == '{') {
                    ...
                    while (j < statement.length && !('}' == statement[j])) {
                        ...
                        if (':' == statement[j] || '{' == statement[j]) {
                            throw new IllegalStateException("Parameter name contains invalid character '" + statement[j] + "' at position "
                                + i + " in statement: " + sql);
                        }
                    }
                    if (j >= statement.length) {
                        throw new IllegalStateException("Non-terminated named parameter declaration at position " + i + " in statement: "
                            + sql);
                    }
                    if (j - i > 3) {
     ...
     */
    public static String substituteNamedParameters(ParsedSql parsedSql, SqlParams paramSource) {
     ...
     */
    /*
     private static SqlParam findParameter(List<SqlParam> declaredParams, String paramName, int paramIndex) {
     if (declaredParams != null) {
     // First pass: Look for named parameter match.
     for (SqlParam declaredParam : declaredParams) {
     ...
     SqlParam declaredParam = declaredParams.get(paramIndex);
     ...
     * @return the SQL statement with substituted parameters
     */
    public static String substituteNamedParameters(String sql, SqlParams paramSource) {
        ParsedSql parsedSql = parseSqlStatement(sql);
    ...
    public static void main(String[] args) {
        ParsedSql sql = NamedParameterUtils.parseSqlStatement("select * from x where x in (:id)");
        SqlParams p = new SqlParams();
        p.setObject("id", Arrays.asList("m", "m"));
        String r = NamedParameterUtils.substituteNamedParameters(sql, p);
